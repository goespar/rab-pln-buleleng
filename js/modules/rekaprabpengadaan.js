// ==========================================
// js/modules/rekaprabpengadaan.js
// Modul Rekap RAB per Pengadaan
// ==========================================

window.renderRekapRabPengadaan = async function renderRekapRabPengadaan() {
    ...
    // Kelompokkan pekerjaan per pengadaan
    const pekerjaanByPengadaan = {};
    allPekerjaan.forEach(p => {
        const pgId = String(p.pengadaan_id || p.id_pengadaan_prk || '');
        if (pgId) {
            if (!pekerjaanByPengadaan[pgId]) pekerjaanByPengadaan[pgId] = [];
            pekerjaanByPengadaan[pgId].push(p);
        }
    });
    ...
    // Build pengadaan list with nested pekerjaan
    const pengadaanWithData = allPengadaan
        .map(pg => {
            ...
            const pekWithRAB = pekerjaanList.map(p => {
                ...
                return { ...p, rabItems, totalMaterial: totals.totalMaterial, totalJasa: totals.totalJasa, totalRAB: totals.totalAll, isJTM, isJTR };
            }).filter(p => p.rabItems.length > 0);
            
            if (pekWithRAB.length === 0) return null;
            
            const pgTotals = calcTotals(pekWithRAB.flatMap(p => p.rabItems));
            return { ...pg, pekerjaan: pekWithRAB, totalMaterial: pgTotals.totalMaterial, totalJasa: pgTotals.totalJasa, totalRAB: pgTotals.totalAll };
        })
        .filter(pg => pg !== null);

    // Filter pengadaan berdasarkan tegangan yang dipilih
    const filteredPengadaan = pengadaanWithData.filter(pg => {
        if (state.rekapTeganganFilter === 'semua') return true;
        const hasJTM = pg.pekerjaan.some(p => p.isJTM);
        const hasJTR = pg.pekerjaan.some(p => p.isJTR);
        if (state.rekapTeganganFilter === 'jtm') return hasJTM;
        if (state.rekapTeganganFilter === 'jtr') return hasJTR;
        return true;
    });
    ...
    contentArea.innerHTML = `
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-xl font-bold text-slate-800">Rekap RAB Pengadaan — Per Komponen Pekerjaan</h2>
                    <p class="text-sm text-slate-500">Rekapitulasi Rencana Anggaran Biaya dikelompokkan berdasarkan Pengadaan dan Komponen Pekerjaan</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-2">
                    <button onclick="printRekapRabPengadaan()" class="bg-blue-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 transition flex items-center gap-2 shadow-sm">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <button onclick="filterRekapByTegangan('semua')" class="px-4 py-2 rounded-lg text-sm font-medium transition ${state.rekapTeganganFilter === 'semua' ? 'bg-brand text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}">
                        <i data-lucide="layers" class="w-4 h-4 inline-block mr-1"></i> Semua (${pekerjaanWithRAB.length})
                    </button>
                    <button onclick="filterRekapByTegangan('jtm')" class="px-4 py-2 rounded-lg text-sm font-medium transition ${state.rekapTeganganFilter === 'jtm' ? 'bg-purple-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}">
                        <i data-lucide="zap" class="w-4 h-4 inline-block mr-1"></i> JTM (${countJTM})
                    </button>
                    <button onclick="filterRekapByTegangan('jtr')" class="px-4 py-2 rounded-lg text-sm font-medium transition ${state.rekapTeganganFilter === 'jtr' ? 'bg-orange-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 print:hidden">
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-3">
                    ...
                    <div><p class="text-xs text-slate-500 font-medium">Total Pekerjaan</p><p class="text-xl font-bold text-slate-800">${pekerjaanWithRAB.length}</p></div>
                </div>
            ...
            <div id="print-content" class="space-y-8">
                ${filteredPengadaan.length === 0 ? '<div class="text-center py-12 text-slate-400 italic">Belum ada data RAB untuk filter yang dipilih</div>' :
                  filteredPengadaan.map((pg, idx) => buildRekapPengadaanKomponenPrint(pg, idx, fmtCur)).join('')}
            </div>

            <!-- Summary -->
            <div class="mt-8 pt-8 border-t-2 border-slate-300 print:break-inside-avoid" data-total-amount="${grandTotalAll}">
                <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="border-l-4 border-slate-400 pl-4">
                            ...
                            <p class="text-2xl font-bold text-slate-800">${fmtCur(grandTotalAll)}</p>
                        </div>
                        <div class="border-l-4 border-blue-500 pl-4">
                            <p class="text-xs text-slate-500 font-medium">DPP (11/12 × Subtotal)</p>
                            <p class="text-2xl font-bold text-blue-700">${fmtCur(grandTotalAll * (11/12))}</p>
                        </div>
                        <div class="border-l-4 border-green-500 pl-4">
                            <p class="text-xs text-slate-500 font-medium">PPN (12% × DPP)</p>
                            <p class="text-2xl font-bold text-green-700">${fmtCur((grandTotalAll * (11/12)) * 0.12)}</p>
                        </div>
                    </div>
                    <div class="mt-6 pt-6 border-t-2 border-slate-300">
                        <div class="bg-emerald-50 rounded-lg p-4 border-2 border-emerald-300 flex justify-between items-center">
                            <span class="text-lg font-bold text-slate-800">TOTAL ANGGARAN (Subtotal + PPN)</span>
                            <span class="text-3xl font-bold text-emerald-700">${fmtCur(Math.round(grandTotalAll + ((grandTotalAll * (11/12)) * 0.12)))}</span>
                        </div>
                    </div>
                </div>
            </div>
...
function buildRekapPengadaanKomponenPrint(pg, pgIdx, fmtCur) {
    const pekerjaan = pg.pekerjaan || [];
    
    // Build table rows dari komponen pekerjaan dengan subtotal
    let tableRows = '';
    let rowNum = 1;
    let pgSubtotal = 0;
    ...
    pekerjaan.forEach(p => {
        ...
        const isJTM = (p.jenis_tegangan || '').toUpperCase() === 'JTM';
        const isJTR = (p.jenis_tegangan || '').toUpperCase() === 'JTR';
        const teganganLabel = isJTM ? 'JTM' : (isJTR ? 'JTR' : '');
        
        // Hitung subtotal untuk pekerjaan ini
        let pekSubtotal = 0;
        items.forEach(item => {
        ...
        pgSubtotal += pekSubtotal;
        
        tableRows += `<tr class="border-b border-slate-200 text-xs hover:bg-slate-50">
            <td class="p-2 text-center w-8 border-r border-slate-200">${rowNum}</td>
            <td class="p-2 border-r border-slate-200 font-medium">${p.nama_pekerjaan || p.nama || '-'}</td>
            <td class="p-2 text-center w-16 border-r border-slate-200">${teganganLabel}</td>
            <td class="p-2 text-center w-12 border-r border-slate-200">${items.length}</td>
            <td class="p-2 text-right w-24 border-r border-slate-200 font-mono">${fmtCur(p.totalMaterial)}</td>
            <td class="p-2 text-right w-24 border-r border-slate-200 font-mono">${fmtCur(p.totalJasa)}</td>
            <td class="p-2 text-right w-24 border-r border-slate-200 font-mono text-blue-600">${fmtCur(p.totalMaterial)}</td>
            <td class="p-2 text-right w-24 border-r border-slate-200 font-mono text-green-600">${fmtCur(p.totalJasa)}</td>
            <td class="p-2 text-right w-28 font-bold font-mono">${fmtCur(pekSubtotal)}</td>
        </tr>`;
        rowNum++;
    });
        ...
        <div class="page-break print:page-break-inside-avoid bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm pengadaan-item" data-pengadaan-id="${pg.id}">
            ...
            <div class="bg-slate-800 text-white px-6 py-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    ...
                    <div><span class="text-slate-300">Total RAB:</span> <span>${fmtCur(pgSubtotal)}</span></div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-100 border-b-2 border-slate-300">
                        <tr class="text-xs font-bold text-slate-700">
                            ...
                            <th class="p-2 border-r border-slate-200">KOMPONEN PEKERJAAN</th>
                            <th class="p-2 text-center w-16 border-r border-slate-200">TEGANGAN</th>
                            <th class="p-2 text-center w-12 border-r border-slate-200">ITEM</th>
                            ...
                            <th class="p-2 text-right w-24 border-r border-slate-200 bg-blue-50">TOTAL MATERIAL</th>
                            <th class="p-2 text-right w-24 border-r border-slate-200 bg-green-50">TOTAL JASA</th>
                            <th class="p-2 text-right w-28">SUBTOTAL (Rp)</th>
                        </tr>
                    </thead>
                    ...
                    <tfoot class="bg-slate-100 border-t-2 border-slate-300 font-bold text-xs">
                        <tr>
                            ...
                            <td class="p-2 text-right font-mono w-24 border-r border-slate-200"></td>
                            <td class="p-2 text-right font-mono w-24 border-r border-slate-200"></td>
                            <td class="p-2 text-right font-mono w-24 border-r border-slate-200"></td>
                            <td class="p-2 text-right font-mono w-24 border-r border-slate-200"></td>
                            <td class="p-2 text-right font-mono w-28"></td>
                        </tr>
                        <tr>
                            <td colspan="8" class="p-2 text-right border-r border-slate-200">JUMLAH:</td>
                            <td class="p-2 text-right font-mono w-28">${fmtCur(pgSubtotal)}</td>
                        </tr>
                        <tr>
                            <td colspan="8" class="p-2 text-right border-r border-slate-200">DPP (11/12):</td>
                            <td class="p-2 text-right font-mono w-28">${fmtCur(dpp)}</td>
                        </tr>
                        <tr>
                            <td colspan="8" class="p-2 text-right border-r border-slate-200">PPN (12%):</td>
                            <td class="p-2 text-right font-mono w-28">${fmtCur(ppn)}</td>
                        </tr>
                        <tr class="bg-emerald-50 border-t-2 border-emerald-300">
                            <td colspan="8" class="p-2 text-right border-r border-slate-200 text-emerald-700">TOTAL ANGGARAN:</td>
                            <td class="p-2 text-right font-mono text-emerald-700 font-bold text-sm w-28">${fmtCur(total)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
function filterRekapByTegangan(tipe) {
    state.rekapTeganganFilter = tipe;
    renderRekapRabPengadaan();
}

function printRekapRabPengadaan() {
    const printContent = document.getElementById('print-content');
    if (!printContent) return showToast('Data RAB tidak ditemukan', 'error');
    
    // Get all pengadaan items
    const pengadaanItems = document.querySelectorAll('.pengadaan-item');
    if (pengadaanItems.length === 0) return showToast('Tidak ada data pengadaan', 'error');
    
    // Build pengadaan selection modal
    let pengadaanOptions = '';
    pengadaanItems.forEach((item, idx) => {
        const nama = item.querySelector('.pengadaan-nama')?.textContent || 'Pengadaan ' + (idx + 1);
        const id = item.dataset.pengadaanId;
        pengadaanOptions += `<option value="${id}">${nama}</option>`;
    });
    
    // Show modal untuk pilih pengadaan
    const modalHtml = `
        <div id="print-modal" style="position:fixed; inset:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; z-index:9999;">
            <div style="background:white; border-radius:12px; padding:24px; max-width:500px; width:90%; box-shadow:0 20px 25px rgba(0,0,0,0.15);">
                <h3 style="font-size:18px; font-weight:bold; margin-bottom:16px; color:#1f2937;">Pilih Pengadaan untuk Diprint</h3>
                <select id="pengadaan-select" style="width:100%; padding:8px 12px; border:1px solid #d1d5db; border-radius:8px; font-size:14px; margin-bottom:20px;">
                ...
                <div style="display:flex; gap:12px; justify-content:flex-end;">
                    ...
                    <button onclick="executePrintRekap()" style="padding:8px 16px; background:#0369a1; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:500;">Print</button>
                </div>
            </div>
        </div>
    `;
    ...
    document.getElementById('pengadaan-select').focus();
}

function executePrintRekap() {
    const selectedPengadaanId = document.getElementById('pengadaan-select').value;
    document.getElementById('print-modal').remove();
    
    // Get data for selected pengadaan or all
    let pengadaanElements = [];
    if (selectedPengadaanId) {
        pengadaanElements = [document.querySelector(`[data-pengadaan-id="${selectedPengadaanId}"]`)];
    } else {
        pengadaanElements = Array.from(document.querySelectorAll('.pengadaan-item'));
    }
    
    if (pengadaanElements.length === 0 || !pengadaanElements[0]) {
        return showToast('Data pengadaan tidak ditemukan', 'error');
    }
    
    // Build print template
    const printTemplate = buildPrintTemplate(pengadaanElements);
